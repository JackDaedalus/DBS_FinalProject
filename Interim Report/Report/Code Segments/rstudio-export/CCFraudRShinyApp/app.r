...
server <- function(input, output, session) {
  ...
  get.arg.list.from.cctrxn <- function(){
    ...
    # Do not include CardOpID - start reading cc trxn record on second entry in row
    i <- 2
    while(i<(len_list+1)){  # This Reflects the number of attribute entries in the cc trxn record  
  ...
  idText <- eventReactive(input$apiTxnRow, {
    ...
    #selected_cc_trxns <- data()[v$s,]
    #selected_cc_trxns$CardOperationsId
    ...
    # Pass Parameters for API Call
    do.call(api_call$AZURE_CreditCardFraud_APICall_withParams$PrintFraudModelResult, api.arg.list.score)
  ...
  ntext <- eventReactive(input$apiBtn, {
    ...
    # Pass Parameters for API Call
    do.call(api_call$AZURE_CreditCardFraud_APICall_withParams$PrintFraudModelResult, api.arg.list)
